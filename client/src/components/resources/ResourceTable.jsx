...
export const ResourceTable = ({ resources, onReport, onAddToCollection, onResourceDeleted }) => {
  ...
  const renderResourceTypeIcon = (type) => {
    switch (type) {
      case 'VIDEO': return <Video className="w-3.5 h-3.5 text-sky-400" />;
  ...
  return (
    <div className="w-full overflow-hidden rounded-2xl border border-slate-800 glass-card">
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs text-slate-300 border-collapse">
          <thead>
            <tr className="border-b border-slate-800 bg-dark-900/90 text-slate-400 font-semibold uppercase tracking-wider text-[11px]">
              <th className="py-3.5 px-4 min-w-[280px]">Content & Title</th>
              <th className="py-3.5 px-3 min-w-[140px]">Source Domain</th>
              <th className="py-3.5 px-3 min-w-[120px]">Category & Type</th>
              <th className="py-3.5 px-3 min-w-[200px]">Direct URL</th>
              <th className="py-3.5 px-3 text-center min-w-[90px]">Stats</th>
              ...
              return (
                <tr
                  ...
                  className={`hover:bg-slate-800/40 transition-colors group ${
                    isHidden ? 'bg-rose-950/10 opacity-75' : ''
                  }`}
                >
                  {/* Content & Title */}
                  <td className="py-3.5 px-4">
                    <div className="flex items-center gap-3">
                      ...
                      <Link
                        ...
                        className="w-12 h-9 rounded-lg overflow-hidden bg-dark-900 shrink-0 border border-slate-700/60 relative block"
                      >
                        {resource.thumbnail ? (
                          <img
                      ...
                      <div className="min-w-0 max-w-sm">
                        <Link
                          to={`/resources/${resource._id}`}
                          className="font-semibold text-slate-100 group-hover:text-sky-300 transition-colors line-clamp-1 block text-xs sm:text-sm"
                        >
                          {resource.title}
                        </Link>
                        ...
                        {Array.isArray(resource.tags) && resource.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {resource.tags.slice(0, 2).map((t, idx) => (
                              <span key={idx} className="text-[9px] font-mono text-slate-400 bg-dark-800/90 px-1.5 py-0.2 rounded border border-slate-800">
                                #{t}
                              </span>
                            ))}
                            {resource.tags.length > 2 && (
                              <span className="text-[9px] text-slate-500">+{resource.tags.length - 2}</span>
                            )}
                          </div>
                        )}
                      </div>
                  ...
                  <td className="py-3.5 px-3">
                    <div className="flex items-center gap-1.5 text-xs text-slate-300">
                      ...
                      />
                      <span className="truncate max-w-[120px] font-medium">{resource.domain}</span>
                    </div>
                  </td>
                  ...
                  <td className="py-3.5 px-3">
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className={`flex items-center gap-1 px-2.5 py-1 rounded-lg border text-[11px] font-medium transition-all cursor-pointer ${
                          isCopied
                            ...
                            : 'bg-dark-800 hover:bg-sky-500/20 text-slate-300 hover:text-sky-300 border-slate-700/60'
                        }`}
                      ...
                      <a
                        ...
                        className="p-1 rounded-lg bg-dark-800 hover:bg-slate-700 text-slate-400 hover:text-slate-200 border border-slate-700/60 transition-colors"
                  ...
                  <td className="py-3.5 px-3 text-center">
                    <div className="flex items-center justify-center gap-2 text-[11px] text-slate-400">
                      ...
                      <span className="flex items-center gap-0.5" title="Saves">
                        <Bookmark className={`w-3 h-3 ${isSaved ? 'text-sky-400 fill-sky-400' : 'text-slate-500'}`} />
